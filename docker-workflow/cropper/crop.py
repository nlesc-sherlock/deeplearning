...
def crop(item, cclass, basedir, output_folder):
    """
...
if __name__ == '__main__':
    ...
    if not specialised:
        for cclass in data['classes']:
            class_crop_folder = os.path.join('cropped', slugify(cclass))
            if len(data['classes'][cclass]) > 0:
                ...
                for item in data['classes'][cclass]:
                    if item['probability'] > args.probability:
                        crop(item, cclass, args.input_directory, class_crop_folder)
    else:
        if 'person' in data['classes'].keys():
            face_crop_folder = os.path.join('cropped', 'face')
            if not os.path.isdir(face_crop_folder):
                os.mkdir(face_crop_folder)
            for person in data['classes']['person']:
                if 'face' in person:
                    crop(person['face'], 'face', args.input_directory, face_crop_folder)
